...
class ScenePooling(nn.Module):
    ...
    def forward(self, episode_idx, trajectory, feature_map, oom_val):
        ...
        trajectory_mapCS_tb = trajectory_mapCS.reshape(-1, 2)
        ...
        # Generate duplicated batch indices for prediction length
    ...
    def social_pooling(self, hidden, position, num_tgt_trajs):
        ...
        for i in range(batch_size):
            ...
            if num_pooling:
                ...
                theta = torch.atan2(y_diff[ego_idx, surround_idx, candidate_idx],
                                    x_diff[ego_idx, surround_idx, candidate_idx])
                ...
                linearized_idx = global_ego_idx * (
                            self.num_rings * self.num_wedges * num_candidates) + candidate_idx * (
                                             self.num_rings * self.num_wedges) + surround_ring_idx * self.num_rings + surround_wedge_idx
        ...
        if len(global_surround_idx_list):
            ...
            sp_tensors = ts.scatter_mean(hidden_pooling, linearized_idx_list,
                                         dim_size=num_agents * num_candidates * self.num_rings * self.num_wedges)
            ...
            sp_tensors = hidden.new_full((num_agents, num_candidates, self.num_rings, self.num_wedges, hidden_size),
                                         0.0)
