...
class R2P2_SimpleRNN:
    ...
    def forward(self, src_trajs_or_src_encoding, ):
        def forward(self, src_trajs_or_src_encoding, episode_idx, decode_start_vel, decode_start_pos,
            ...
            mu = []
            sigma = []
            z = torch.normal(mean=0.0, std=1.0, size=(total_agents * self.num_candidates, self.decoding_steps * 2),
                             device=device)
            ...
            x = torch.stack(x, dim=1).reshape(total_agents, self.num_candidates, self.decoding_steps,
                                              2)  # x: Na X Nc X Td X 2
            z = z.reshape(total_agents, self.num_candidates, self.decoding_steps * 2)
            mu = torch.stack(mu, dim=1).reshape(total_agents, self.num_candidates, self.decoding_steps,
                                                2)  # mu: Na X Nc X Td X 2
            sigma = torch.stack(sigma, dim=1).reshape(total_agents, self.num_candidates, self.decoding_steps, 2,
...
class R2P2_RNN(nn.Module):
    ...
    def forward(self, src_trajs_or_src_encoding, episode_idx, decode_start_vel, decode_start_pos,
                scene_or_scene_encoding, motion_encoded=False, scene_encoded=False):
        """
        Na: total # of agents
        Nb: batch size (# episodes)
        Nc: # candidates
